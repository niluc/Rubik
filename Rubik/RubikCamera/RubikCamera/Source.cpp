...
public:
    ...
    int color2num() {
        ...
        double h = this->HSV[0] / 2;
        double s = this->HSV[1] * 255 / 100;
        int color;
...
static vector<node> drawSquares(Mat& image, const vector<vector<Point>>& squares)
{
    ...
    for (size_t i = 0; i < squares.size(); i++)
    {
        ...
        if ((float)r.width / (float)r.height < 1.2 && (float)r.height / (float)r.width < 1.2) {
            rectangle(image, rect, Scalar(0, 255, 0), 2, LINE_AA);
    ...
    for (int i = 0; i < list.size(); i++) {
        ...
        if (sqrt(a * a + b * b) <= 2 * (list[index].size / 2 + list[i].size / 2)) {
            ...
            //cout << sqrt(a * a + b * b) / (float)(list[index].size + list[i].size) << endl;
        }
    }
    ...
    if (listFace.size() == 9) {
        ...
        sort(pnt, pnt + 9, compareY);
        sort(pnt, pnt + 3, compareX);
        sort(pnt + 3, pnt + 6, compareX);
        sort(pnt + 6, pnt + 9, compareX);
...
static vector<node> sqColor(Mat& image, int w, int h) {
...
extern "C" {
    _declspec(dllexport) int* cameraInput()
    {
        const int n = 1;
        namedWindow("Rubik Camera Input", WINDOW_AUTOSIZE);
        int mode = 0;
        int index = 0;
        static int map[54];

        vector<node> tmp, cur, cur1, list;
        ...
        for (int t = 0; t < 6; t++) {
            ...
            {
                cap.release();
                destroyWindow("Rubik Camera Input");
                return NULL;
            }
            ...
            while (true)
            {
                ...
                //imshow("Rubik Camera Input", frame);
            ...
            if (allow == 1) {
                ...
                sort(pnt1, pnt1 + 9 * n, compareY);
                sort(pnt1, pnt1 + 3 * n, compareX);
                sort(pnt1 + 3 * n, pnt1 + 6 * n, compareX);
                sort(pnt1 + 6 * n, pnt1 + 9 * n, compareX);

                cout << endl;
                for (int i = 0; i < 9; i++) {
                    cout << pnt1[i].point << endl;
                }

                cout << endl;
                for (int k = 0; k < 9 * n; k++) {
                    //cout << pnt1[k].point << pnt1[k].HSV << pnt1[k].BGR << color << endl;
                    cout << pnt1[k].color2num() << " " << pnt1[k].HSV[0] / 2 << endl;
